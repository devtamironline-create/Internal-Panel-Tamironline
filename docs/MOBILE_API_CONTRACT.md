...
> **UPDATE 2026-08-25 — کاتالوگِ استان‌محور (جواب نامهٔ تیم اپ — تصمیم کارفرما: استان، نه شهر):**
> - دانه‌بندی فیلتر **استان** است: پوشش در هر شهری از استان ⇒ کلِ استان آن خدمت را می‌گیرد (لباسشویی در مشهد ⇒ نیشابور هم می‌بیند). همان `city_id` را بفرستید — سرور استانش را مبنا می‌گیرد؛ `state_id` مستقیم هم پذیرفته است.
> - `GET /v1/customer/services/categories?city_id=N` (یا `?state_id=P`) → فقط دستگاه‌های تحتِ پوششِ استان (تگ تکنسین‌ها + کنترل «پوشش خدمات» پنل). بدون پارامتر = رفتار قبلی. استانِ بدون پوشش/شهرِ نامعتبر = آرایهٔ خالی (۲۰۰). تا `coverage_data_complete=false` هیچ محدودیتی اعمال نمی‌شود.
> - `GET /v1/customer/services/brands?device_id=D&city_id=N` → اجتماعِ برندهای دارای تکنسین در استان برای آن دستگاه؛ تکنسینِ بدونِ تگ برند = همهٔ برندها (بی‌اثر). `objections` منطقه‌ای نیست — `city_id` نادیده گرفته می‌شود.
> - `GET /v1/customer/locations/states` و `/cities` → فیلد `serviceable: bool` در **سطح استان** (شهرِ بدونِ تکنسین در استانِ تحتِ پوشش هم true است). موارد false را در انتخاب نشان ندهید. تا کامل‌شدنِ دادهٔ پوشش همه true.
> - `GET /v1/customer/status` → فیلد جدید `default_city: {id, name} | null` — فقط با Bearer token معتبر: شهرِ آدرسِ پیش‌فرض/آخرینِ مشتری برای پیش‌انتخاب.
> - هر تغییرِ فعال/غیرفعالی در «پوشش خدمات» پنل، `cache_version` در `/status` را جلو می‌برد — کش `["categories", city_id]` را باطل کنید.
