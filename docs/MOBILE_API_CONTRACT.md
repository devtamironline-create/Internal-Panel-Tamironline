...
> **UPDATE 2026-08-25 — کاتالوگِ شهرمحور (جواب نامهٔ تیم اپ):**
> - `GET /v1/customer/services/categories?city_id=N` → فقط دستگاه‌های تحتِ پوششِ آن شهر (تگ تکنسین‌ها + کنترل «پوشش خدمات» پنل). بدون `city_id` = رفتار قبلی. شهرِ بدون پوشش/نامعتبر = آرایهٔ خالی (۲۰۰). تا `coverage_data_complete=false` هیچ محدودیتی اعمال نمی‌شود.
> - `GET /v1/customer/services/brands?device_id=D&city_id=N` → اگر در آن شهر فقط برندهای خاصی برای دستگاه تکنسین دارند، محدود به همان‌ها؛ وگرنه `city_id` بی‌اثر است. `objections` شهری نیست — `city_id` نادیده گرفته می‌شود.
> - `GET /v1/customer/locations/cities` → فیلد جدید `serviceable: bool` (شهر حداقل یک خدمتِ قابل‌ارائه دارد). شهرهای `serviceable=false` را در انتخابِ شهرِ کاتالوگ نشان ندهید. تا کامل‌شدنِ دادهٔ پوشش همه true.
...
> - هر تغییرِ فعال/غیرفعالیِ per-city در پنل («پوشش خدمات»)، `cache_version` در `/status` را جلو می‌برد — کش `["categories", city_id]` را باطل کنید.
