...
| **تیکتِ پشتیبانی** (+`ticket-categories`) | ✅ | ✅ | **پیاده‌سازی شد** |
| **چت با اپراتور** (+صفحه‌بندی +read) | ✅ | ✅ | **پیاده‌سازی شد** |
| **اعلانات** | ✅ | ✅ | **پیاده‌سازی شد** |

> **تیکت/چت/اعلانات اکنون در API موبایل هم موجودند** (`docs/TECH_APP_API.md` بخش ۱۲).
> تنها شکافِ باقی‌مانده: **ورود با رمز** (فقط وب) و کانالِ **WebSocket/Push** (آینده؛
> فعلاً polling).
