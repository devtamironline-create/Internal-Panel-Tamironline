...
### نتیجهٔ تماس
- `result ∈ {coordinated, no_answer}`؛ اگر `no_answer` → `reason` اجباری (حداقل ۳ حرف). `coordinated`→ `next_action='schedule_visit'` (+`has_default_time`)؛ `no_answer`→ در فازِ هماهنگی وضعیت «مشتری پاسخگو نیست». روی سفارشِ نهایی قفل.
...
## ۱۱) چت با اپراتور — فقط وب (API ندارد) ⚠️

| قابلیت | وب | API |
|---|---|---|
| صفحهٔ گفتگو (+علامتِ خوانده‌شدن) | `GET /tech/messages` | ❌ |
| ارسالِ پیام | `POST /tech/messages/send` | ❌ |
| Poll پیام‌های جدید | `GET /tech/messages/poll?after_id=` | ❌ |
| شمارِ نخوانده (بَج) | `GET /tech/messages/unread` | ❌ |

- محدود به `technician_id`. اگر اپراتوری تخصیص نیافته → ارسال `422`. `body` اجباری ≤۲۰۰۰. `poll` پیام‌های `id > after_id` را می‌دهد و پیام‌های اپراتور را «خوانده» می‌کند.
