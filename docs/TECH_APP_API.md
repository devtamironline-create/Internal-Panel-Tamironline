...
**الزامِ UX:** فقط وقتی سفارش در **فازِ هماهنگی** است (`new` /
`awaiting_coordination` / `no_answer`) و تکنسین روی `tel:` می‌زند و به اپ
برمی‌گردد، مودالِ **اجباری** «نتیجهٔ تماس؟» نشان دهید (تشخیصِ برگشت با
`AppState`/`visibilitychange` + پشتیبانِ تایمر — بدونِ نیاز به رفرش):
- **`{ "result": "coordinated" }`** → `data.next_action = "schedule_visit"` و
  `data.has_default_time` (bool). فرمِ «ثبتِ زمانِ مراجعه» را باز کنید؛ اگر
  `has_default_time=true` (سفارش با روز/ساعت از اپ آمده) تقویم را با زمانِ
  فعلیِ سفارش **پیش‌پر** و فقط دکمهٔ «تأیید» بگذارید؛ اگر `false` تقویمِ خالی
  برای انتخاب. (schedule-visit خودش «هماهنگ شده» + پیامک را انجام می‌دهد.)
- **`{ "result": "no_answer", "reason": "..." }`** → `reason` **الزامی** (حداقل
  ۳ حرف؛ چرا پاسخ نداد). یک باکسِ توضیح در مرحلهٔ دوم بگیرید. در فازِ هماهنگی
