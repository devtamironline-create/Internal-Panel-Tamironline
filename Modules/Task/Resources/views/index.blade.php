...
<div class="space-y-6" x-data="kanbanBoard()">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">مدیریت تسک‌ها</h1>
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('admin.messenger') }}?conversation={{ $teamConversationId }}" class="inline-flex items-center gap-2 px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-2">
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('tasks.index', ['team' => $team->slug]) }}"
                ...
                {{ $team->name }}
                <span class="text-xs bg-{{ $currentTeam && $currentTeam->id === $team->id ? $team->color.'-200 dark:'.$team->color.'-800' : 'gray-200 dark:bg-gray-700' }} px-2 py-0.5 rounded-full">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-4">
        <form method="GET" action="{{ route('tasks.index') }}" class="flex flex-wrap items-center gap-4">
            ...
            <div class="flex items-center gap-2">
                <label class="text-sm text-gray-600 dark:text-gray-400">مسئول:</label>
                <select name="assignee" onchange="this.form.submit()" class="text-sm rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-brand-500 focus:ring-brand-500">
            ...
            <div class="flex items-center gap-2">
                <label class="text-sm text-gray-600 dark:text-gray-400">اولویت:</label>
                <select name="priority" onchange="this.form.submit()" class="text-sm rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-brand-500 focus:ring-brand-500">
                    <option value="">همه</option>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="kanban-column bg-gray-100 dark:bg-gray-800/50 rounded-xl p-4 min-h-[500px] transition-all"
            data-status="{{ $status }}"
            @dragover.prevent="$event.currentTarget.classList.add('ring-2', 'ring-brand-500', 'ring-opacity-50')"
            @dragleave="$event.currentTarget.classList.remove('ring-2', 'ring-brand-500', 'ring-opacity-50')"
            ...
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-2">
                    ...
                    <h3 class="font-bold text-gray-900 dark:text-white">{{ $column['label'] }}</h3>
                </div>
                <span class="text-sm text-gray-500 dark:text-gray-400">{{ $column['tasks']->count() }}</span>
            </div>
    ...
    <div x-show="showCreateModal" x-cloak
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <!-- Modal Content -->
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-lg mx-auto z-10"
                x-transition:enter="transition ease-out duration-300"
                ...
                <div class="flex items-center justify-between p-5 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">تسک جدید</h3>
                ...
                <form @submit.prevent="saveTask()" class="p-5 space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">عنوان تسک *</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">مسئول انجام</label>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">ددلاین</label>
    ...
    <div x-show="showTaskModal" x-cloak
        ...
        <div class="flex items-start justify-center min-h-screen px-4 py-8">
            ...
            <!-- Modal Content -->
            <div class="relative bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-2xl mx-auto z-10"
                x-transition:enter="transition ease-out duration-300"
                x-transition:enter-start="opacity-0 transform scale-95"
                x-transition:enter-end="opacity-100 transform scale-100">
                ...
                <!-- Task Content -->
                <div x-show="!loadingTask && currentTask" class="divide-y divide-gray-200 dark:divide-gray-700">
                    <!-- Modal Header -->
                    <div class="p-5">
                            ...
                            <div class="flex-1 pl-4">
                                <div class="flex items-center gap-2 mb-2">
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium"
                                        :class="'bg-' + currentTask.priority_color + '-100 dark:bg-' + currentTask.priority_color + '-900/30 text-' + currentTask.priority_color + '-700 dark:text-' + currentTask.priority_color + '-400'"
                                        x-text="currentTask.priority_label"></span>
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium"
                                        :class="'bg-' + currentTask.status_color + '-100 dark:bg-' + currentTask.status_color + '-900/30 text-' + currentTask.status_color + '-700 dark:text-' + currentTask.status_color + '-400'"
                                        x-text="currentTask.status_label"></span>
                                </div>
                                <h3 class="text-xl font-bold text-gray-900 dark:text-white" x-text="currentTask.title"></h3>
                    ...
                    <div class="p-5 bg-gray-50 dark:bg-gray-700/50">
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">تغییر وضعیت</label>
                        <div class="grid grid-cols-4 gap-2">
                            <button @click="changeTaskStatus('todo')"
                                class="px-3 py-2 text-sm rounded-lg transition"
                                :class="currentTask.status === 'todo' ? 'bg-blue-500 text-white' : 'bg-white dark:bg-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-500'">
                                در انتظار
                            </button>
                            <button @click="changeTaskStatus('in_progress')"
                                class="px-3 py-2 text-sm rounded-lg transition"
                                :class="currentTask.status === 'in_progress' ? 'bg-yellow-500 text-white' : 'bg-white dark:bg-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-500'">
                                در حال انجام
                            </button>
                            <button @click="changeTaskStatus('review')"
                                class="px-3 py-2 text-sm rounded-lg transition"
                                :class="currentTask.status === 'review' ? 'bg-purple-500 text-white' : 'bg-white dark:bg-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-500'">
                                بررسی
                            </button>
                            <button @click="changeTaskStatus('done')"
                                class="px-3 py-2 text-sm rounded-lg transition"
                                :class="currentTask.status === 'done' ? 'bg-green-500 text-white' : 'bg-white dark:bg-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-500'">
                                تکمیل
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="p-5" x-show="currentTask.description">
                        <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">توضیحات</h4>
                        <p class="text-gray-600 dark:text-gray-400 text-sm whitespace-pre-line" x-text="currentTask.description"></p>
                    </div>
                    ...
                    <div class="p-5">
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div class="flex justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                ...
                                <span class="font-medium text-gray-900 dark:text-white" x-text="currentTask.assignee_name || 'بدون مسئول'"></span>
                            </div>
                            <div class="flex justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                                <span class="text-gray-500 dark:text-gray-400">ددلاین</span>
                                <span class="font-medium" :class="currentTask.is_overdue ? 'text-red-600 dark:text-red-400' : 'text-gray-900 dark:text-white'"
                    ...
                    <div class="p-5" x-show="currentTask.checklists && currentTask.checklists.length > 0">
                        <div class="flex items-center justify-between mb-3">
                            <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300">چک‌لیست</h4>
                            <span class="text-xs text-gray-500 dark:text-gray-400" x-text="currentTask.checklist_completed + '/' + currentTask.checklist_total"></span>
                        </div>
                            ...
                            <template x-for="item in currentTask.checklists" :key="item.id">
                                <label class="flex items-center gap-3 p-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg cursor-pointer">
                                    <input type="checkbox" :checked="item.is_completed"
                                        ...
                                        class="rounded text-green-600 focus:ring-green-500 dark:bg-gray-600 dark:border-gray-500">
                                    <span :class="item.is_completed ? 'line-through text-gray-400' : 'text-gray-700 dark:text-gray-300'" x-text="item.title"></span>
                                </label>
                            </template>
                        </div>
                    </div>
                    ...
                    <!-- Comments -->
                    <div class="p-5">
                        <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">نظرات</h4>
                        ...
                        <div class="flex gap-2 mb-4">
                            <input type="text" x-model="newComment"
                                ...
                                class="flex-1 text-sm rounded-lg border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white focus:border-brand-500 focus:ring-brand-500">
                            ...
                            <template x-for="comment in currentTask.comments" :key="comment.id">
                                <div class="flex gap-3">
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <p class="text-gray-600 dark:text-gray-400 text-sm" x-text="comment.body"></p>
                                    </div>
                                </div>
                            </template>
                            <p x-show="!currentTask.comments || currentTask.comments.length === 0" class="text-center text-gray-500 dark:text-gray-400 py-2 text-sm">
                                هنوز نظری ثبت نشده
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="p-5 bg-gray-50 dark:bg-gray-700/50 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-3">
                            <a :href="'/tasks/' + currentTask.id + '/edit'" class="px-4 py-2 bg-gray-100 dark:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-500 text-sm">
                                ویرایش کامل
                            </a>
...
function kanbanBoard() {
    return {
        ...
        handleDragStart(event, taskId) {
            ...
            event.target.classList.add('opacity-50', 'cursor-grabbing');
            event.dataTransfer.effectAllowed = 'move';
            event.dataTransfer.setData('text/plain', taskId);
        },

        handleDragEnd(event) {
            event.target.classList.remove('opacity-50', 'cursor-grabbing');
            // Remove highlight from all columns
            document.querySelectorAll('.kanban-column').forEach(col => {
                col.classList.remove('ring-2', 'ring-brand-500', 'ring-opacity-50');
            });
        ...
        handleDrop(event, newStatus) {
            ...
            event.currentTarget.classList.remove('ring-2', 'ring-brand-500', 'ring-opacity-50');

            const taskId = this.draggedTaskId || event.dataTransfer.getData('text/plain');
            if (!taskId) return;

            // Show loading indicator on the task card
            const taskCard = document.querySelector(`[data-task-id="${taskId}"]`);
            if (taskCard) {
                taskCard.classList.add('opacity-50', 'pointer-events-none');
            }
            ...
            .then(data => {
                if (data.success) {
                    // Move card visually before reload
                    const targetColumn = document.querySelector(`#column-${newStatus}`);
                    if (taskCard && targetColumn) {
                        targetColumn.appendChild(taskCard);
                        taskCard.classList.remove('opacity-50', 'pointer-events-none');
                    }
