...
class AiModerationService
{
    ...
    private function userPrompt(string $text, array $context): string
    {
        ...
        // زمینهٔ کامل (مقاله/سوال + گفتگوی قبلی) — بدونِ آن، پیامِ کوتاهِ مرتبط
        // (مثلِ «عالی بود مشکل حل شد») بی‌ربط به نظر می‌رسد و به‌اشتباه رد می‌شود.
        $ctx = isset($context['context']) && trim((string) $context['context']) !== ''
            ? "── زمینه (صفحه/سوال و گفتگوی قبلی — فقط برای قضاوتِ بهتر، خودش را طبقه‌بندی نکن) ──\n{$context['context']}\n────────────────────────────────────────────────\n\n"
            : '';
        ...
        قواعدِ مهم:
        - «بی‌ربط بودن» را فقط با توجه به زمینهٔ بالا قضاوت کن — نه به‌تنهایی.
        - پیامِ کوتاهِ تشکر/تأیید یا ادامهٔ گفتگو که با زمینه مرتبط است (مثلِ
          «عالی بود، مشکل حل شد») کاملاً سالم است → "approve". کوتاه‌بودن یا
          نداشتنِ جزئیات به‌هیچ‌وجه دلیلِ رد نیست.
        - اگر مطمئن نیستی، "approve" با confidenceِ پایین بده.

        دربارهٔ "reason": یک جملهٔ کوتاهِ فارسیِ «محترمانه و خطاب به کاربر» بنویس که
        مستقیماً به خودِ نویسنده نشان داده می‌شود — مخصوصاً برای spam/reject علتِ
        دقیقِ رد را بگو (مثلاً «متنِ شما حاویِ شماره‌تماسِ تبلیغاتی است» یا «لطفاً
        بدونِ الفاظِ نامناسب دوباره ارسال کنید»)، نه توضیحِ فنیِ طبقه‌بندی.
