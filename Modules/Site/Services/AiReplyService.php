...
class AiReplyService
{
    ...
    {
        return 'تو کارشناسِ تعمیراتِ «تعمیرآنلاین» (خدماتِ تخصصیِ تعمیرِ لوازم خانگی) هستی. '
            .'به نظر/سوالِ کاربر یک پاسخِ فارسیِ کوتاه (۲ تا ۴ جمله)، مؤدب، دقیق و کاربردی بده. '
            .'قوانین: فقط بر اساسِ زمینه‌ای که در همین پیام داده می‌شود (مقاله/سوال/گفتگوی قبلی) پاسخ بده؛ '
            .'مطالبِ پاسخ‌های قبلی را تکرار نکن و در صورتِ نیاز به آن‌ها ارجاع بده؛ '
            .'قیمت یا زمانِ دقیق را قطعی نگو (بگو پس از بررسی/بازدید اعلام می‌شود)؛ '
    ...
    private function userPrompt(string $text, array $context): string
    {
        ...
        // رشتهٔ گفتگو (سوالِ اصلی + پاسخ‌های قبلی) — برای پیگیری‌های انجمن، تا
        // پاسخِ جدید در ادامهٔ همان گفتگو باشد نه از صفر.
        $thread = isset($context['thread'])
            ? "گفتگوی پیشینِ این رشته (برای زمینه):\n{$context['thread']}\n────────────\n"
            : '';
        // زمینهٔ کامل (مقاله/سوال + زنجیرهٔ گفتگو، ساخته‌شده با AiContext) — مدل
        // بینِ فراخوانی‌ها حافظه ندارد؛ همه‌چیز باید در همین prompt باشد.
        $ctx = isset($context['context']) && trim((string) $context['context']) !== ''
            ? "زمینهٔ کامل (فقط برای فهمِ بهتر — به خودِ زمینه پاسخ نده):\n{$context['context']}\n────────────\n"
            : '';

        return "به این «{$type}»ِ کاربر پاسخ بده:\n{$ctx}{$thread}{$page}{$title}«««\n{$text}\n»»»";
    }
