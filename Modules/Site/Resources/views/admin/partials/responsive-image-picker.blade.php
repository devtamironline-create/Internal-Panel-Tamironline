{{--
    Responsive image picker — دسکتاپ + موبایل با media library integration.
    استفاده در _field.blade.php (نوع responsive_image) و هر جای دیگری که
    دو تصویر با alt مجزا نیاز باشد.

    Required vars:
      $name        — اسم فیلد (مثلاً sections[hero][payload][image])
      $desktopUrl  — URL فعلی دسکتاپ (string|null)
      $desktopAlt  — alt فعلی دسکتاپ (string|null)
      $mobileUrl   — URL فعلی موبایل (string|null)
      $mobileAlt   — alt فعلی موبایل (string|null)
--}}
@php
    $compId = 'rpick_'.substr(md5($name.uniqid()), 0, 6);
@endphp
...
<div x-data="{{ $compId }}({
        ...
        initial: {
            desktop: { url: @js(old($name.'.desktop.url', $desktopUrl)), alt: @js(old($name.'.desktop.alt', $desktopAlt)) },
            mobile:  { url: @js(old($name.'.mobile.url',  $mobileUrl)),  alt: @js(old($name.'.mobile.alt',  $mobileAlt)) },
        },
     })" class="space-y-4">

    <p class="text-xs text-gray-500">
        برای هر اسلات می‌توانید تصویر متفاوت با متن جایگزین (alt) مستقل تنظیم کنید — حتی اگر URL یکسان باشد.
        این برای SEO و دسترسی‌پذیری مفید است.
    </p>
