...
/**
 * قالبِ Titleِ سئوی مقالات: فقط «{عنوانِ کوتاهِ مقاله}» — بدونِ برند.
 *
 ...
 * استانداردِ سئو (~۶۰ کاراکتر) رد می‌شوند. این هلپر همهٔ آن پسوندها — از جمله
 * خودِ «تعمیرآنلاین» — را از Title حذف می‌کند و طول را محدود می‌کند. idempotent
 * است: اگر ورودی از قبل تمیز باشد، خروجی تغییری نمی‌کند.
 *
...
final class ArticleSeoTitle
{
    ...
    public static function build(?string $rawTitle, int $max = self::MAX): string
    {
        ...
        if ($base === '') {
            // اگر پاک‌سازی عنوانِ معناداری نگذاشت (مثلاً ورودی فقط برند/پسوند بوده)،
            // از عنوانِ خامِ تریم‌شده استفاده کن تا Title خالی نماند.
            $base = trim((string) $rawTitle);
        }

        if (mb_strlen($base) > $max) {
            $base = self::truncateWords($base, $max);
        }
