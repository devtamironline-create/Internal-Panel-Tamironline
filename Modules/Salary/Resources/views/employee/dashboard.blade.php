...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <a href="{{ route('salary.history') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">
    ...
    <div class="bg-gradient-to-r from-brand-600 to-brand-700 rounded-xl p-6 text-white">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                ...
                <h2 class="text-2xl font-bold mt-1">{{ $currentSalary['period_label'] }}</h2>
                <p class="text-brand-200 text-sm mt-2">روز {{ $currentSalary['current_day'] }} از {{ $currentSalary['days_in_month'] }} روز</p>
            </div>
            <div class="text-center md:text-left">
                <p class="text-brand-200 text-sm">حقوق خالص تخمینی</p>
                <p class="text-3xl font-bold mt-1">{{ number_format($currentSalary['estimated_net']) }}</p>
        ...
        <div class="mt-4">
            <div class="flex justify-between text-sm text-brand-200 mb-1">
                ...
                <span>{{ round(($currentSalary['current_day'] / $currentSalary['days_in_month']) * 100) }}%</span>
            </div>
            <div class="w-full bg-brand-800 rounded-full h-2">
                <div class="bg-white rounded-full h-2 transition-all" style="width: {{ ($currentSalary['current_day'] / $currentSalary['days_in_month']) * 100 }}%"></div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white rounded-xl p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-bold text-gray-900">{{ $currentSalary['work_days'] }}</p>
        ...
        <div class="bg-white rounded-xl p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-bold text-gray-900">{{ $currentSalary['work_hours'] }}</p>
        ...
        <div class="bg-white rounded-xl p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-bold text-gray-900">{{ $currentSalary['overtime_hours'] }}</p>
        ...
        <div class="bg-white rounded-xl p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <p class="text-2xl font-bold text-gray-900">{{ $currentSalary['late_time'] }}</p>
        ...
        <div class="bg-white rounded-xl shadow-sm">
            ...
            <div class="p-4 space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-medium">{{ number_format($currentSalary['fixed_insurance_salary']) }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">بن مسکن</span>
                    <span class="font-medium">{{ number_format($currentSalary['housing_allowance']) }}</span>
                </div>
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">حق خواروبار</span>
                    <span class="font-medium">{{ number_format($currentSalary['food_allowance']) }}</span>
                </div>
                @if($currentSalary['marriage_allowance'] > 0)
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">حق تأهل</span>
                    <span class="font-medium">{{ number_format($currentSalary['marriage_allowance']) }}</span>
                </div>
                @endif
                @if($currentSalary['child_allowance'] > 0)
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">حق اولاد</span>
                    <span class="font-medium">{{ number_format($currentSalary['child_allowance']) }}</span>
                </div>
                @endif
                @if($currentSalary['monthly_non_insurance'] > 0)
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">مابه‌التفاوت توافقی</span>
                    <span class="font-medium">{{ number_format($currentSalary['monthly_non_insurance']) }}</span>
                </div>
                @endif
                @if($currentSalary['total_overtime'] > 0)
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">اضافه‌کاری</span>
                    <span class="font-medium text-green-600">+{{ number_format($currentSalary['total_overtime']) }}</span>
                </div>
                @endif
                <div class="pt-3 border-t border-gray-100 flex justify-between items-center">
                    <span class="font-semibold text-gray-900">جمع مزایا</span>
                    <span class="font-bold text-green-600">{{ number_format($currentSalary['total_benefits'] + $currentSalary['total_overtime'] + $currentSalary['monthly_non_insurance']) }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm">
            ...
            <div class="p-4 space-y-3">
                <div class="flex justify-between items-center">
                    ...
                    <span class="font-medium text-red-600">-{{ number_format($currentSalary['employee_insurance']) }}</span>
                </div>
                @if($currentSalary['late_penalty'] > 0)
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">جریمه تاخیر</span>
                    <span class="font-medium text-red-600">-{{ number_format($currentSalary['late_penalty']) }}</span>
                </div>
                @endif
                @if($currentSalary['used_leave'] > 0)
                <div class="flex justify-between items-center">
                    <span class="text-gray-600">مرخصی استفاده شده</span>
                    <span class="font-medium text-red-600">-{{ number_format($currentSalary['used_leave']) }}</span>
                </div>
                @endif
                <div class="pt-3 border-t border-gray-100 flex justify-between items-center">
                    <span class="font-semibold text-gray-900">جمع کسورات</span>
                    <span class="font-bold text-red-600">{{ number_format($currentSalary['employee_insurance'] + $currentSalary['late_penalty'] + $currentSalary['used_leave']) }}</span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <div class="text-center md:text-left">
                <p class="text-4xl font-bold text-brand-600">{{ number_format($currentSalary['estimated_net']) }}</p>
