...
@section('page-title', 'اهداف سازمانی')
@section('main')
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        @can('manage-okr')
        <a href="{{ route('okr.objectives.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-brand-600 text-white rounded-lg hover:bg-brand-700">
    ...
    <div class="bg-white rounded-xl shadow-sm p-4">
        <form action="{{ route('okr.objectives.index') }}" method="GET" class="flex flex-wrap gap-4">
            <select name="cycle_id" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500">
                <option value="">همه دوره‌ها</option>
                @foreach($cycles as $cycle)
                <option value="{{ $cycle->id }}" {{ request('cycle_id') == $cycle->id ? 'selected' : '' }}>{{ $cycle->title }}</option>
                @endforeach
            </select>
            <select name="level" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500">
                <option value="">همه سطوح</option>
                <option value="organization" {{ request('level') === 'organization' ? 'selected' : '' }}>سازمانی</option>
                <option value="team" {{ request('level') === 'team' ? 'selected' : '' }}>تیمی</option>
                <option value="individual" {{ request('level') === 'individual' ? 'selected' : '' }}>فردی</option>
            </select>
            ...
            @if(request()->hasAny(['cycle_id', 'level', 'status']))
            <a href="{{ route('okr.objectives.index') }}" class="px-4 py-2 text-gray-500 hover:text-gray-700">پاک کردن</a>
            @endif
        </form>
    </div>
            ...
            <div class="p-4 hover:bg-gray-50 transition">
                <div class="flex items-start justify-between gap-4">
                    ...
                    <div class="flex items-center gap-4">
                        <div class="text-left">
                            <p class="text-2xl font-bold {{ $objective->progress >= 70 ? 'text-green-600' : ($objective->progress >= 40 ? 'text-yellow-600' : 'text-red-600') }}">{{ number_format($objective->progress, 0) }}%</p>
