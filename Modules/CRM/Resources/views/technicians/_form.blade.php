                ...
                @foreach($allDevices as $d)
                    <label class="flex items-center gap-1.5 cursor-pointer text-sm">
                        ...
                        <input type="number" name="device_priority[{{ $d->id }}]" min="0" max="9" dir="ltr"
                               ...
                               title="اولویت تخصص (۰ تا ۹) — با عددِ همین دستگاه نزدِ تکنسین‌های دیگر مقایسه می‌شود"
                               class="w-11 px-1 py-0.5 text-xs text-center border border-gray-200 dark:border-gray-600 dark:bg-gray-700 rounded">
                    </label>
                @endforeach
            </div>
            <div class="text-[11px] text-gray-500 dark:text-gray-400 leading-6 mt-2 p-2 rounded-lg bg-gray-50 dark:bg-gray-700/40 border border-gray-200 dark:border-gray-600">
                <b class="text-gray-700 dark:text-gray-200">عدد کنار هر دستگاه = اولویت تخصص (۰ تا ۹)</b>
                <div class="mt-1">
                    این عدد <b>بین تکنسین‌ها</b> مقایسه می‌شود، نه بین دستگاه‌های خودِ این تکنسین.
                    یعنی وقتی سفارش «جاروبرقی» می‌رسد، عددِ جاروبرقیِ این تکنسین با عددِ جاروبرقیِ
                    بقیه سنجیده می‌شود. پس عددها فقط وقتی معنا دارند که برای همهٔ تکنسین‌ها با یک
                    قاعده پر شوند.
                </div>
                <div class="mt-1">
                    فقط <b>ترتیب</b> مهم است نه اندازه: ۵ در برابر ۳ دقیقاً مثل ۹ در برابر ۱ عمل
                    می‌کند.
                </div>
                <div class="mt-1">
                    این عدد <b>چرخش پخش کار را بر هم نمی‌زند</b>. اول نوبت و کمترین کار فعال حساب
                    می‌شود؛ اولویت تخصص فقط وقتی وارد می‌شود که دو تکنسین بار و نوبت یکسان داشته
                    باشند. پس متخصصِ یک دستگاه همهٔ سفارش‌های آن را جمع نمی‌کند.
                </div>
                <div class="mt-1 text-gray-600 dark:text-gray-300">
                    قاعدهٔ پیشنهادی — همین را برای همهٔ تکنسین‌ها به کار ببرید:
                    <span class="inline-block mt-1">
                        <b>۰</b> بدون ترجیح (پیش‌فرض) ·
                        <b>۳</b> بلد است و کارش خوب است ·
                        <b>۵</b> تخصص اصلی‌اش همین است ·
                        <b>۷+</b> فقط برای موارد استثنایی
                    </span>
                </div>
            </div>
        </div>
