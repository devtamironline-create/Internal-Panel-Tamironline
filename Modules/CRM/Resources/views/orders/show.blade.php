...
<div class="p-6 space-y-6" x-data="{ showNotes: false, showReceipts: false }">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center justify-start sm:justify-end gap-2">
            ...
            {{-- دکمه مشاهده پیش‌فاکتورهای این سفارش — فقط وقتی حداقل یکی وجود دارد. --}}
            @if(($proformaCount ?? 0) > 0)
            <a href="{{ route('crm.proformas.index', ['order_id' => $order->id]) }}"
               class="px-4 py-2 bg-white dark:bg-gray-700 border border-indigo-300 dark:border-indigo-600 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-50 dark:hover:bg-gray-600 rounded-lg inline-flex items-center gap-2 text-sm font-bold"
               title="نمایش پیش‌فاکتورهای صادرشده برای این سفارش">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                مشاهده پیش‌فاکتورها ({{ $proformaCount }})
            </a>
            @endif
            @endcan
