...
@section('page-title', 'ویرایش برند — ' . $brand->name)
...
<div x-data="entityFormPage()" class="min-h-screen bg-gray-50 dark:bg-gray-900">
        ...
        <div class="p-4 flex items-center justify-between gap-4">
            <div class="flex-1 min-w-0">
                ...
                <h1 class="text-lg font-bold text-gray-900 dark:text-white truncate flex items-center gap-2">
                    @if($brand->logo)
                        <img src="{{ $brand->logo }}" alt="" class="w-7 h-7 object-contain rounded">
                    @endif
                    <span>ویرایش برند: {{ $brand->name }}</span>
                </h1>
            </div>
            <div class="flex items-center gap-2 shrink-0">
                <button type="button" @click="expandAll()" class="px-3 py-2 text-xs bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 rounded-lg" title="باز کردن همه سکشن‌ها">باز همه</button>
                <button type="button" @click="collapseAll()" class="px-3 py-2 text-xs bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 rounded-lg" title="بستن همه سکشن‌ها">بسته همه</button>
    ...
    <div class="p-4 lg:p-6">
        <div class="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
                ...
                <div class="sticky top-24">
                    <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
                        <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                            <h3 class="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wide">سکشن‌ها</h3>
                        </div>
            ...
            {{-- Form --}}
            <form id="brand-form" action="{{ route('crm.brands.update', $brand) }}" method="POST" enctype="multipart/form-data">
                @method('PUT')
