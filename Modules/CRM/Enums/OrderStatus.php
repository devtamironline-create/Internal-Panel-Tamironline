...
enum OrderStatus: string
{
     ...
     *
     * پیش‌فاکتور پس از «هماهنگ‌شدن» و پیش از بسته‌شدن معنا دارد: در فازِ
     * تماس/هماهنگیِ اولیه (جدید/در انتظارِ هماهنگی/عدم پاسخ) هنوز زمانِ
     * مراجعه قطعی نیست، و بعد از بسته‌شدن سفارش سندِ مالی فاکتور است نه
     * پیش‌فاکتور. «هماهنگ‌شده» اکنون پیش‌فاکتور می‌پذیرد (خواستهٔ ۱۴۰۵/۰۶)
     * چون تکنسین می‌تواند پیش از مراجعه/شروعِ کار برآوردِ اولیه بدهد.
     *
    ...
    {
        return ! $this->isFinal()
            && ! in_array($this, [self::New, self::AwaitingCoordination, self::NoAnswer], true);
    }
