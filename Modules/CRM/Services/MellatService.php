...
class MellatService
{
    ...
    protected function soapCall(string $method, array $params): array
    {
        ...
        try {
            // endpoint بدون ?wsdl برای فراخوانی واقعی SOAP
            $endpoint = preg_replace('/\?wsdl$/i', '', $this->wsdlEndpoint);
            ...
            // استخراج محتوای <return> — با پشتیبانی از namespace prefix
            // (مثل <ns2:return> یا <return>). این علت اصلی شکست نسخهٔ قبلی
            // بود: اگر پاسخ prefix داشت، regex قدیمی مطابقت نمی‌کرد و
            // verify/settle انجام نمی‌شد → بانک تراکنش را reverse می‌کرد.
            if (preg_match('/<(?:[\w.-]+:)?return\b[^>]*>(.*?)<\/(?:[\w.-]+:)?return>/s', $xml, $m)) {
                return ['ok' => true, 'return' => html_entity_decode(trim($m[1])), 'raw' => $xml];
            }

            // اگر SOAP Fault بود، پیامش را استخراج کن
            if (preg_match('/<(?:[\w.-]+:)?faultstring[^>]*>(.*?)<\/(?:[\w.-]+:)?faultstring>/s', $xml, $f)) {
                Log::warning("Mellat {$method} SOAP Fault", ['fault' => $f[1], 'http' => $response->status()]);
                return ['ok' => false, 'message' => 'خطای درگاه ملت: ' . trim($f[1]), 'raw' => $xml];
            }

            Log::warning("Mellat {$method} — no <return> in response", [
                'http' => $response->status(),
                'xml' => mb_substr($xml, 0, 800),
            ]);
