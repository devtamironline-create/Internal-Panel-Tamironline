...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm">
        ...
        <form action="{{ isset($staff) ? route('admin.staff.update', $staff) : route('admin.staff.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox" id="can_add_group_members" name="can_add_group_members" value="1" {{ old('can_add_group_members', $staff->can_add_group_members ?? false) ? 'checked' : '' }} class="w-4 h-4 text-brand-600 border-gray-300 rounded focus:ring-brand-500">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <label class="relative cursor-pointer">
                        <input type="radio" name="warehouse_role" value="{{ $value }}"
                            class="peer sr-only"
                            {{ old('warehouse_role', $staff->warehouse_role ?? '') === $value ? 'checked' : '' }}>
