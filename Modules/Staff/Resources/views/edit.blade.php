    ...
    <div class="bg-white rounded-xl shadow-sm">
        ...
        <form action="{{ isset($staff) ? route('admin.staff.update', $staff) : route('admin.staff.store') }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <select name="role" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500" required>
                    <option value="">انتخاب کنید...</option>
                    @foreach($roles as $role)
                    <option value="{{ $role->name }}" {{ old('role', isset($staff) ? $staff->roles->first()?->name : '') === $role->name ? 'selected' : '' }}>
                        @switch($role->name)
                            @case('admin') مدیر سیستم @break
                            @case('manager') مدیر @break
                            @case('supervisor') سرپرست @break
                            @case('staff') کارمند @break
                            @default {{ $role->name }}
                        @endswitch
                    </option>
                    @endforeach
            ...
            @if(isset($staff) && isset($permissions))
            <!-- Permissions Section -->
            <div class="pt-6 border-t border-gray-200">
                <h3 class="text-lg font-semibold text-gray-900 mb-2">دسترسی‌های اضافی</h3>
                <p class="text-sm text-gray-500 mb-4">دسترسی‌هایی که علاوه بر نقش انتخاب شده به این کاربر اختصاص می‌یابد</p>
