...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm p-4">
        <form action="{{ route('admin.staff.index') }}" method="GET" class="flex flex-col sm:flex-row gap-4">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="جستجو نام، نام خانوادگی یا موبایل..." class="flex-1 px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500">
            <button type="submit" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200">جستجو</button>
            @if(request('search'))
            <a href="{{ route('admin.staff.index') }}" class="px-4 py-2 bg-gray-50 text-gray-600 rounded-lg hover:bg-gray-100">پاک کردن</a>
            @endif
        </form>
    </div>
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-full bg-brand-100 flex items-center justify-center text-brand-600 font-medium">{{ mb_substr($member->first_name ?? 'U', 0, 1) }}</div>
                            <div>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center gap-2">
                            <a href="{{ route('admin.staff.edit', $member) }}" class="p-2 text-gray-600 hover:text-brand-600 hover:bg-brand-50 rounded-lg" title="ویرایش">
