...
@section('page-title', 'ویرایش سرویس')
@section('main')
<div class="max-w-4xl">
    <div class="bg-white rounded-xl shadow-sm">
        <div class="p-6 border-b border-gray-100">
            <h2 class="text-lg font-semibold text-gray-900">ویرایش سرویس: {{ $service->order_number }}</h2>
        </div>
        <form action="{{ route('admin.services.update', $service) }}" method="POST" class="p-6 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="billing_cycle" id="billing_cycle" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 @error('billing_cycle') border-red-500 @enderror" required>
                        ...
                        <option value="monthly" {{ old('billing_cycle', $service->billing_cycle) == 'monthly' ? 'selected' : '' }}>ماهانه</option>
                        <option value="quarterly" {{ old('billing_cycle', $service->billing_cycle) == 'quarterly' ? 'selected' : '' }}>سه ماهه</option>
                        <option value="semiannually" {{ old('billing_cycle', $service->billing_cycle) == 'semiannually' ? 'selected' : '' }}>شش ماهه</option>
                        <option value="annually" {{ old('billing_cycle', $service->billing_cycle) == 'annually' ? 'selected' : '' }}>سالانه</option>
                        <option value="biennially" {{ old('billing_cycle', $service->billing_cycle) == 'biennially' ? 'selected' : '' }}>دو سالانه</option>
                        <option value="onetime" {{ old('billing_cycle', $service->billing_cycle) == 'onetime' ? 'selected' : '' }}>یکباره</option>
                        <option value="hourly" {{ old('billing_cycle', $service->billing_cycle) == 'hourly' ? 'selected' : '' }}>ساعتی</option>
                    </select>
                ...
                <div>
                    ...
                    <input type="text" name="start_date"
                        value="{{ old('start_date', $service->start_date ? \Morilog\Jalali\Jalalian::fromDateTime($service->start_date)->format('Y/m/d') : '') }}"
            ...
            <div class="border-t pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="number" name="price" id="price" value="{{ old('price', $service->price) }}" min="0" step="1000"
                    ...
                    <div>
                        ...
                        <input type="number" name="setup_fee" id="setup_fee" value="{{ old('setup_fee', $service->setup_fee) }}" min="0" step="1000"
                    ...
                    <div>
                        ...
                        <input type="number" name="discount_amount" value="{{ old('discount_amount', $service->discount_amount) }}" min="0" step="1000"
                            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
            ...
            <div>
                ...
                <textarea name="notes" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500" placeholder="یادداشت‌های داخلی درباره این سرویس...">{{ old('notes', $service->notes) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 border-t pt-6">
                <div>
                    ...
                    <select name="status" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 @error('status') border-red-500 @enderror" required>
                        <option value="pending" {{ old('status', $service->status) == 'pending' ? 'selected' : '' }}>در انتظار</option>
                        <option value="active" {{ old('status', $service->status) == 'active' ? 'selected' : '' }}>فعال</option>
                        <option value="suspended" {{ old('status', $service->status) == 'suspended' ? 'selected' : '' }}>تعلیق</option>
                        <option value="cancelled" {{ old('status', $service->status) == 'cancelled' ? 'selected' : '' }}>لغو شده</option>
                        <option value="expired" {{ old('status', $service->status) == 'expired' ? 'selected' : '' }}>منقضی</option>
                    </select>
                    @error('status')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                </div>

                <div class="flex items-center pt-7">
                    <input type="checkbox" id="auto_renew" name="auto_renew" value="1" {{ old('auto_renew', $service->auto_renew) ? 'checked' : '' }}
            ...
            <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                <div class="grid grid-cols-2 gap-4 text-sm">
                    <div>
                        ...
                        <span class="font-medium text-gray-900 mr-2">{{ $service->order_number }}</span>
                    </div>
                    <div>
                        <span class="text-gray-600">تاریخ تمدید بعدی:</span>
...
document.getElementById('product_id').addEventListener('change', function() {
    ...
    const setupFee = selectedOption.getAttribute('data-setup-fee');

    if (basePrice && confirm('آیا می‌خواهید قیمت را به قیمت پایه محصول تغییر دهید؟')) {
        document.getElementById('price').value = basePrice;
    }
    if (setupFee && confirm('آیا می‌خواهید هزینه راه‌اندازی را به مقدار پیش‌فرض تغییر دهید؟')) {
