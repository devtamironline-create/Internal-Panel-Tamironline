...
<div class="p-6" x-data="ordersPage()">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-100">سفارشات</h1>
            <p class="text-slate-400 mt-1">مدیریت و پیگیری سفارشات ووکامرس</p>
        </div>
    ...
    <div class="bg-slate-800 rounded-lg p-4 mb-6">
        <form method="GET" action="{{ route('warehouse.orders.index') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-4">
            <!-- Search -->
            <div>
                <label class="block text-sm text-slate-400 mb-1">جستجو</label>
            ...
            <!-- WooCommerce Status -->
            <div>
                <label class="block text-sm text-slate-400 mb-1">وضعیت ووکامرس</label>
                <select name="status" class="w-full bg-slate-700 border-slate-600 rounded-lg text-slate-200 text-sm focus:ring-blue-500 focus:border-blue-500">
            ...
            <!-- Internal Status -->
            <div>
                <label class="block text-sm text-slate-400 mb-1">وضعیت داخلی</label>
                <select name="internal_status" class="w-full bg-slate-700 border-slate-600 rounded-lg text-slate-200 text-sm focus:ring-blue-500 focus:border-blue-500">
                    <option value="">همه</option>
            ...
            <!-- Shipped Status -->
            <div>
                <label class="block text-sm text-slate-400 mb-1">وضعیت ارسال</label>
                <select name="is_shipped" class="w-full bg-slate-700 border-slate-600 rounded-lg text-slate-200 text-sm focus:ring-blue-500 focus:border-blue-500">
                    <option value="">همه</option>
            ...
            <!-- Assigned To -->
            <div>
                <label class="block text-sm text-slate-400 mb-1">مسئول</label>
                <select name="assigned_to" class="w-full bg-slate-700 border-slate-600 rounded-lg text-slate-200 text-sm focus:ring-blue-500 focus:border-blue-500">
                    <option value="">همه</option>
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('warehouse.orders.index') }}" class="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg transition">
                    پاک
                </a>
            </div>
        </form>
    ...
    <div x-show="selectedOrders.length > 0"
         x-transition
         class="bg-blue-900/50 border border-blue-700 rounded-lg p-4 mb-4 flex items-center justify-between">
        <span class="text-blue-300">
            <span x-text="selectedOrders.length"></span> سفارش انتخاب شده
        </span>
        <div class="flex items-center gap-2">
            <select x-model="bulkAction" class="bg-slate-700 border-slate-600 rounded-lg text-slate-200 text-sm">
            ...
            <select x-show="bulkAction === 'update_internal_status'" x-model="bulkValue" class="bg-slate-700 border-slate-600 rounded-lg text-slate-200 text-sm">
                @foreach($internalStatuses as $value => $label)
            ...
            <table class="w-full">
                <thead class="bg-slate-700/50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right text-xs font-medium text-slate-400">شماره سفارش</th>
                        <th class="px-4 py-3 text-right text-xs font-medium text-slate-400">مشتری</th>
                        <th class="px-4 py-3 text-right text-xs font-medium text-slate-400">محصولات</th>
                        ...
                        <th class="px-4 py-3 text-right text-xs font-medium text-slate-400">وضعیت داخلی</th>
                        <th class="px-4 py-3 text-right text-xs font-medium text-slate-400">ارسال</th>
                ...
                <tbody class="divide-y divide-slate-700">
                    ...
                    <tr class="hover:bg-slate-700/50">
                        ...
                        <td class="px-4 py-3">
                            <div class="text-slate-300">{{ $order->customer_full_name }}</div>
                            @if($order->billing_phone)
                            <div class="text-xs text-slate-500">{{ $order->billing_phone }}</div>
                            @endif
                        </td>
                        ...
                        <td class="px-4 py-3">
                            <span class="inline-flex px-2 py-1 text-xs rounded-full bg-{{ $order->internal_status_color }}-900/50 text-{{ $order->internal_status_color }}-400">
                                {{ $order->internal_status_label }}
                            </span>
                        </td>
                        <td class="px-4 py-3">
                            @if($order->is_shipped)
                            <span class="text-green-400" title="{{ $order->tracking_code }}">
                        ...
                        <td class="px-4 py-3 text-slate-400 text-sm">
                            ...
                            <div class="text-xs text-slate-500">{{ $order->date_created?->format('H:i') }}</div>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <a href="{{ route('warehouse.orders.show', $order) }}"
                                   class="p-1.5 text-slate-400 hover:text-slate-200 hover:bg-slate-700 rounded transition"
                                   title="مشاهده">
