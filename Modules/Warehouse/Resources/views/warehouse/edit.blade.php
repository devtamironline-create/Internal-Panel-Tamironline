...
<div class="space-y-6">
    <div class="flex items-center gap-4">
        ...
        <div>
            <h1 class="text-xl font-bold text-gray-900">ویرایش سفارش</h1>
            <p class="text-gray-600 mt-1">شماره سفارش: <span dir="ltr" class="text-brand-600 font-medium">{{ $order->order_number }}</span></p>
        </div>
    </div>

    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('warehouse.update', $order) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="customer_mobile" value="{{ old('customer_mobile', $order->customer_mobile) }}" dir="ltr"
                ...
                <div>
                    ...
                    <select name="shipping_type" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                        ...
                        <option value="{{ $type->slug }}" {{ old('shipping_type', $order->shipping_type) == $type->slug ? 'selected' : '' }}>{{ $type->name }} ({{ $type->timer_label }})</option>
                        @endforeach
                    </select>
                    @error('shipping_type')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <select name="assigned_to" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                        ...
                        <option value="{{ $user->id }}" {{ old('assigned_to', $order->assigned_to) == $user->id ? 'selected' : '' }}>{{ $user->full_name }}</option>
                        @endforeach
                    </select>
                    @error('assigned_to')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                ...
                <div>
                    ...
                    <input type="text" name="tracking_code" value="{{ old('tracking_code', $order->tracking_code) }}" dir="ltr"
                           class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                    @error('tracking_code')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500">{{ old('description', $order->description) }}</textarea>
                @error('description')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div>
                ...
                <textarea name="notes" rows="2" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500">{{ old('notes', $order->notes) }}</textarea>
                @error('notes')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div class="flex items-center gap-3 pt-4 border-t">
                <button type="submit" class="px-6 py-2.5 bg-brand-600 text-white rounded-lg hover:bg-brand-700 font-medium">ذخیره تغییرات</button>
                <a href="{{ route('warehouse.show', $order) }}" class="px-6 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 font-medium">انصراف</a>
            </div>
        </form>
    </div>
</div>
