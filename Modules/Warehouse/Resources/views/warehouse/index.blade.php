...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('warehouse.manual-order.create') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-brand-600 text-white rounded-lg hover:bg-brand-700 transition-colors">
                    ...
                    <div x-show="showPrintModal" x-transition.scale.origin.center class="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden max-h-[90vh] flex flex-col" @click.stop>
                        ...
                        <div class="px-5 py-4 space-y-4 overflow-y-auto flex-1">
                            ...
                            <div class="p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-700 leading-relaxed">
                                ...
                                بعد از تایید، فاکتور در تب جدید پرینت می‌شود و سفارش به «در صف ارسال (اسکن)» منتقل می‌شود.
                            </div>
                        </div>
                        {{-- دکمه‌ها --}}
...
function pendingOrderCard(orderId, shippingType, shippingLabel, needsPostcode, totalWeightGrams) {
    return {
        ...
        get selectedBoxWeight() {
            if (!this.selectedBox || !window._boxSizesData) return 0;
            var box = window._boxSizesData[this.selectedBox];
