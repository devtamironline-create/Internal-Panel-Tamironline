...
<div class="p-6" x-data="packingStation()">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-slate-100">ایستگاه بسته‌بندی</h1>
            <p class="text-slate-400 mt-1">اسکن بارکد سفارش و محصولات</p>
        </div>
    ...
    <div class="grid lg:grid-cols-2 gap-6">
        ...
        <div class="space-y-6">
            ...
            <div class="bg-slate-800 rounded-lg p-4">
                <div class="flex items-center justify-between mb-4">
                    <h2 class="font-semibold text-slate-100">اسکنر بارکد</h2>
                ...
                <div class="flex gap-2">
                    <input type="text"
                           ...
                           class="flex-1 bg-slate-700 border-slate-600 rounded-lg text-slate-200 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div class="mt-4 p-3 rounded-lg"
                     :class="currentOrder ? 'bg-emerald-900/30 border border-emerald-700' : 'bg-blue-900/30 border border-blue-700'">
                    <div class="flex items-center gap-2">
                        <div class="w-3 h-3 rounded-full animate-pulse"
                             :class="currentOrder ? 'bg-emerald-400' : 'bg-blue-400'"></div>
                        <span class="text-sm" :class="currentOrder ? 'text-emerald-300' : 'text-blue-300'"
            ...
            <!-- Last Scan Result -->
            <div x-show="lastScanResult" x-transition class="bg-slate-800 rounded-lg p-4">
                <h3 class="text-sm font-medium text-slate-400 mb-3">آخرین اسکن</h3>
                <div class="p-3 rounded-lg"
                     :class="lastScanResult?.success ? 'bg-green-900/30 border border-green-700' : 'bg-red-900/30 border border-red-700'">
                    <div class="flex items-center gap-3">
                        <div :class="lastScanResult?.success ? 'text-green-400' : 'text-red-400'">
                        ...
                        <div>
                            <p class="font-medium" :class="lastScanResult?.success ? 'text-green-300' : 'text-red-300'"
                               x-text="lastScanResult?.message"></p>
                            <p class="text-sm text-slate-400" x-text="'کد: ' + lastScanResult?.code"></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Weight Entry -->
            <div x-show="currentOrder && allItemsScanned" x-transition class="bg-slate-800 rounded-lg p-4">
                <h3 class="font-semibold text-slate-100 mb-4">ثبت وزن بسته</h3>
                <p class="text-sm text-slate-400 mb-4">همه محصولات اسکن شدند. حالا وزن بسته را وارد کنید.</p>

                <div class="space-y-4">
                    @if(isset($settings['default_carton_weight']) && $settings['default_carton_weight'] > 0)
                    <div class="flex justify-between text-sm">
                        <span class="text-slate-500">وزن کارتن پیش‌فرض:</span>
                        <span class="text-slate-300">{{ $settings['default_carton_weight'] }} گرم</span>
                    </div>
                    @endif

                    <div>
                        <label class="block text-sm text-slate-400 mb-1">وزن بسته (گرم)</label>
                        <input type="number"
                               x-model="packageWeight"
                               placeholder="وزن نهایی بسته"
                               min="0"
                               class="w-full bg-slate-700 border-slate-600 rounded-lg text-slate-200 text-lg focus:ring-blue-500 focus:border-blue-500">
                    </div>
        ...
        <div class="space-y-6">
            <!-- Order Details -->
            <div x-show="currentOrder" x-transition class="bg-slate-800 rounded-lg">
                <div class="p-4 border-b border-slate-700 flex items-center justify-between">
                    <div>
                        <h2 class="font-semibold text-slate-100">سفارش <span x-text="'#' + currentOrder?.order_number"></span></h2>
                        <p class="text-sm text-slate-400" x-text="currentOrder?.customer_name"></p>
                    </div>
                    <button @click="cancelOrder()"
                            class="px-3 py-1 bg-slate-700 hover:bg-slate-600 text-slate-300 rounded-lg text-sm transition">
                        لغو
                    </button>
                </div>
                ...
                <div class="p-4 border-b border-slate-700">
                    <div class="flex items-center justify-between text-sm mb-2">
                        <span class="text-slate-400">پیشرفت اسکن</span>
                ...
                <div class="divide-y divide-slate-700 max-h-[400px] overflow-y-auto">
                    <template x-for="item in orderItems" :key="item.id">
                        <div class="p-4 flex items-center gap-4"
                             :class="item.is_scanned ? 'bg-green-900/20' : ''">
                            <!-- Status Icon -->
                            <div class="w-8 h-8 rounded-full flex items-center justify-center shrink-0"
                                 :class="item.is_scanned ? 'bg-green-500/20 text-green-400' : 'bg-slate-700 text-slate-500'">
                            ...
                            <div class="flex-1 min-w-0">
                                <p class="font-medium text-slate-200 truncate" x-text="item.name"></p>
                                <div class="flex items-center gap-3 mt-1 text-sm">
                                    <span class="text-slate-500">SKU: <span class="text-slate-400" x-text="item.sku || '-'"></span></span>
                                    <span class="text-slate-500">تعداد: <span class="text-slate-400" x-text="item.quantity"></span></span>
                                </div>
                                <div x-show="item.bin_location" class="mt-1">
                                    <span class="text-xs bg-blue-900/50 text-blue-400 px-2 py-0.5 rounded" x-text="'قفسه: ' + item.bin_location"></span>
                                </div>
                            </div>
                            ...
                            <div class="text-left">
                                <div class="text-lg font-bold"
                                     :class="item.scanned_quantity >= item.quantity ? 'text-green-400' : 'text-slate-300'">
                                    <span x-text="item.scanned_quantity"></span>/<span x-text="item.quantity"></span>
                                </div>
                                <div class="text-xs text-slate-500">اسکن شده</div>
                            </div>
                        </div>
                    </template>
                </div>
            ...
            <!-- No Order Selected -->
            <div x-show="!currentOrder" class="bg-slate-800 rounded-lg p-8 text-center">
                <svg class="w-16 h-16 mx-auto text-slate-600 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"/>
                </svg>
                <h3 class="text-lg font-medium text-slate-300 mb-2">سفارشی انتخاب نشده</h3>
                <p class="text-slate-500">بارکد سفارش را اسکن کنید یا شماره سفارش را وارد کنید</p>
            </div>

            <!-- Recent Completed Orders -->
            <div class="bg-slate-800 rounded-lg">
                <div class="p-4 border-b border-slate-700">
                    <h3 class="font-semibold text-slate-100">بسته‌بندی‌های اخیر</h3>
                </div>
                <div class="p-4 space-y-3">
                    ...
                    <div class="flex items-center justify-between p-3 bg-slate-700/50 rounded-lg">
                        <div>
                            ...
                            <p class="text-sm text-slate-400">{{ $order->customer_full_name }}</p>
                        </div>
                        <div class="text-left">
                            @if($order->weight_verified)
                            <span class="text-green-400 text-sm">وزن تایید</span>
                            @else
                            <span class="text-yellow-400 text-sm">اختلاف وزن</span>
                            @endif
                            <p class="text-xs text-slate-500">{{ $order->updated_at->diffForHumans() }}</p>
                        </div>
                    </div>
                    @empty
                    <p class="text-center text-slate-500 py-4">بسته‌بندی ثبت نشده</p>
                    @endforelse
                </div>
            </div>
        </div>
    ...
    <div x-show="showWeightWarning"
        ...
        <div class="bg-slate-800 rounded-lg p-6 max-w-md mx-4">
            <div class="flex items-center gap-3 mb-4">
                ...
                <h3 class="text-xl font-semibold text-slate-100">هشدار اختلاف وزن!</h3>
            </div>
            <div class="space-y-3 mb-6">
                <p class="text-slate-300">وزن ثبت شده با وزن مورد انتظار مطابقت ندارد.</p>
                <div class="bg-slate-700/50 rounded-lg p-3 space-y-2">
                    <div class="flex justify-between">
                        <span class="text-slate-400">وزن مورد انتظار:</span>
                        <span class="text-slate-200 font-medium" x-text="expectedWeight + ' گرم'"></span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-slate-400">وزن ثبت شده:</span>
                        <span class="text-slate-200 font-medium" x-text="packageWeight + ' گرم'"></span>
                    </div>
                    <div class="flex justify-between border-t border-slate-600 pt-2">
                        <span class="text-slate-400">اختلاف:</span>
                        <span class="text-yellow-400 font-bold" x-text="weightDifferencePercent + '%'"></span>
                    </div>
                </div>
                <p class="text-yellow-400 text-sm">احتمالاً محصولی جا مانده یا اشتباه اضافه شده است!</p>
            </div>
    ...
    <div x-show="showSuccessModal"
         x-transition
         class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
        <div class="bg-slate-800 rounded-lg p-6 max-w-md mx-4 text-center">
            <div class="w-16 h-16 bg-green-500/20 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg class="w-10 h-10 text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                </svg>
            </div>
            <h3 class="text-xl font-semibold text-slate-100 mb-2">بسته‌بندی تکمیل شد!</h3>
            <p class="text-slate-400 mb-6" x-text="'سفارش #' + completedOrderNumber + ' با موفقیت بسته‌بندی شد.'"></p>
