...
<div class="p-6" x-data="courierManager()">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">مدیریت پیک</h1>
            <p class="text-gray-500 dark:text-gray-400 mt-1">سفارشات آماده ارسال با پیک</p>
        </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    ...
                    <div class="text-sm text-gray-500 dark:text-gray-400">در انتظار تخصیص</div>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ $stats['assigned'] }}</div>
                    <div class="text-sm text-gray-500 dark:text-gray-400">تخصیص داده شده</div>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-2xl font-bold text-green-600 dark:text-green-400">{{ $stats['delivered_today'] }}</div>
                    <div class="text-sm text-gray-500 dark:text-gray-400">تحویل امروز</div>
                </div>
            </div>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-lg p-4 shadow-sm">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <div class="text-2xl font-bold text-purple-600 dark:text-purple-400">{{ count($couriers) }}</div>
                    <div class="text-sm text-gray-500 dark:text-gray-400">پیک‌های فعال</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
            <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                <h2 class="font-semibold text-gray-900 dark:text-white">سفارشات در انتظار تخصیص</h2>
                <span class="px-3 py-1 bg-orange-100 dark:bg-orange-900/50 text-orange-700 dark:text-orange-400 rounded-full text-sm">
                    {{ $pendingOrders->count() }} سفارش
                </span>
            </div>
            <div class="divide-y divide-gray-200 dark:divide-gray-700 max-h-[500px] overflow-y-auto">
                @forelse($pendingOrders as $order)
                <div class="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition" x-data="{ expanded: false }">
                    <div class="flex items-start justify-between">
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-1">
                                <a href="{{ route('warehouse.orders.show', $order) }}" class="font-bold text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300">
                                    #{{ $order->order_number }}
                                </a>
                                @if($order->is_packed)
                                <span class="px-2 py-0.5 bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-400 text-xs rounded">بسته‌بندی شده</span>
                                @endif
                            </div>
                            <p class="text-sm text-gray-700 dark:text-gray-300">{{ $order->customer_full_name }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-500 mt-1">{{ $order->shipping_city ?? $order->billing_city }}</p>
                        </div>
                        <div class="text-left">
                            <p class="font-medium text-gray-900 dark:text-gray-200">{{ $order->formatted_total }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-500">{{ $order->date_created?->diffForHumans() }}</p>
                        </div>
                    </div>
                    ...
                    <div class="mt-3 pt-3 border-t border-gray-200 dark:border-gray-700">
                        <div class="flex gap-2">
                            <select x-model="selectedCourier_{{ $order->id }}"
                                    class="flex-1 bg-gray-50 dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-900 dark:text-gray-200 focus:ring-blue-500 focus:border-blue-500">
                        ...
                        <div x-show="selectedCourier_{{ $order->id }} === 'new'" x-transition class="mt-3 space-y-2">
                            <input type="text"
                                   ...
                                   placeholder="نام پیک"
                                   class="w-full bg-gray-50 dark:bg-gray-700 border-gray-300 dark:border-gray-600 rounded-lg text-sm text-gray-900 dark:text-gray-200 focus:ring-blue-500 focus:border-blue-500">
                            <input type="tel"
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                <div class="p-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                    <h3 class="font-semibold text-gray-900 dark:text-white">پیک‌های فعال</h3>
                ...
                <div class="divide-y divide-gray-200 dark:divide-gray-700">
                    ...
                    <div class="p-4">
                        <div class="flex items-center justify-between">
                            <div>
                                <p class="font-medium text-gray-900 dark:text-gray-200">{{ $courier['name'] }}</p>
                                <p class="text-sm text-gray-500 dark:text-gray-400">{{ $courier['mobile'] }}</p>
                            </div>
                            <div class="text-left">
                                <span class="px-2 py-1 bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-400 text-xs rounded">
                    ...
                    @empty
                    <div class="p-4 text-center text-gray-500 dark:text-gray-500 text-sm">
                        پیکی ثبت نشده
                    </div>
                    @endforelse
                </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                <div class="p-4 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="font-semibold text-gray-900 dark:text-white">تخصیص‌های امروز</h3>
                </div>
                <div class="p-4 space-y-3 max-h-[300px] overflow-y-auto">
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                        <div>
                            ...
                            <p class="text-xs text-gray-500 dark:text-gray-400">{{ $order->courier_name }}</p>
                        </div>
                        <div class="text-left">
                            @if($order->internal_status === 'delivered')
                            <span class="text-green-600 dark:text-green-400 text-xs">تحویل شده</span>
                            @else
                            <span class="text-blue-600 dark:text-blue-400 text-xs">در مسیر</span>
                            @endif
                        </div>
                    </div>
                    @empty
                    <p class="text-center text-gray-500 dark:text-gray-500 text-sm">تخصیصی امروز ثبت نشده</p>
                    @endforelse
                </div>
            </div>
        </div>
    ...
    <div x-show="showAddCourier"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md mx-4 w-full">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">افزودن پیک جدید</h3>
            <div class="space-y-4">
                <div>
                    <label class="block text-sm text-gray-500 dark:text-gray-400 mb-1">نام و نام خانوادگی</label>
                    <input type="text"
                ...
                <div>
                    <label class="block text-sm text-gray-500 dark:text-gray-400 mb-1">شماره موبایل</label>
                    <input type="tel"
