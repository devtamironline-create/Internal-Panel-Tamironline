...
<div class="space-y-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <a href="{{ route('attendance.settings') }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form action="{{ route('attendance.admin') }}" method="GET" class="flex flex-wrap items-end gap-4">
            <div>
                ...
                <input type="date" name="date" value="{{ $date }}" class="rounded-lg border-gray-300 focus:border-blue-500 focus:ring-blue-500">
            </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ $allStaff->count() }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600">{{ count($presentIds) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-red-600">{{ $allStaff->count() - count($presentIds) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-yellow-600">{{ $attendances->where('late_minutes', '>', 0)->count() }}</p>
                </div>
            </div>
        </div>
    </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div class="text-xs text-gray-500">{{ $staff->mobile }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm {{ $attendance && $attendance->check_in ? 'text-green-600 font-medium' : 'text-gray-400' }}">
                                {{ $attendance && $attendance->check_in ? $attendance->check_in : '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm {{ $attendance && $attendance->check_out ? 'text-blue-600 font-medium' : 'text-gray-400' }}">
                                {{ $attendance && $attendance->check_out ? $attendance->check_out : '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                {{ $attendance ? $attendance->work_hours : '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm {{ $attendance && $attendance->late_minutes > 0 ? 'text-red-600 font-medium' : 'text-gray-400' }}">
                                {{ $attendance && $attendance->late_minutes > 0 ? $attendance->late_time : '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm {{ $attendance && $attendance->overtime_minutes > 0 ? 'text-green-600 font-medium' : 'text-gray-400' }}">
                                {{ $attendance && $attendance->overtime_minutes > 0 ? $attendance->overtime_minutes . ' دقیقه' : '-' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                @if($attendance)
