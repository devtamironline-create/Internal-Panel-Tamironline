...
<div class="max-w-4xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600">{{ \App\Http\Controllers\Admin\RoleController::getRoleLabel($role->name) }}</p>
        </div>
            ...
            <div class="grid md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name', $role->name) }}"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-brand-500 focus:border-brand-500 {{ in_array($role->name, ['admin', 'manager', 'supervisor', 'staff']) ? 'bg-gray-100' : '' }}"
