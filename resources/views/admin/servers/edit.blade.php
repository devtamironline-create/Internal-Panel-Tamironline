...
@section('page-title', 'ویرایش سرور')
@section('main')
<div class="mb-6">
...
<div class="bg-white rounded-xl shadow-sm">
    <div class="p-6 border-b border-gray-100">
        <h2 class="text-lg font-semibold text-gray-900">ویرایش سرور: {{ $server->name }}</h2>
    </div>

    <form action="{{ route('admin.servers.update', $server) }}" method="POST" class="p-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select name="type" id="type" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    <option value="shared" {{ old('type', $server->type) == 'shared' ? 'selected' : '' }}>اشتراکی</option>
                    <option value="vps" {{ old('type', $server->type) == 'vps' ? 'selected' : '' }}>سرور مجازی</option>
                    <option value="dedicated" {{ old('type', $server->type) == 'dedicated' ? 'selected' : '' }}>اختصاصی</option>
                    <option value="reseller" {{ old('type', $server->type) == 'reseller' ? 'selected' : '' }}>نمایندگی</option>
                </select>
                @error('type')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="notes" id="notes" rows="3"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('notes') border-red-500 @enderror">{{ old('notes', $server->notes) }}</textarea>
                @error('notes')
                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mt-6 flex gap-3">
            <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">بروزرسانی سرور</button>
