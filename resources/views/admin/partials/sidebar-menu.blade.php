...
<li x-data="{ warehouseOpen: {{ request()->routeIs('warehouse.*') ? 'true' : 'false' }} }">
    ...
    <ul x-show="warehouseOpen && sidebarOpen" x-transition class="mt-1 mr-4 space-y-1">
        ...
        <li>
            <a href="{{ route('warehouse.shipping.index') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg text-slate-400 hover:bg-slate-700 hover:text-white text-sm {{ request()->routeIs('warehouse.shipping.*') ? 'bg-slate-700/50 text-white' : '' }}">
